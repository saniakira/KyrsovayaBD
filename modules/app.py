...
from modules.constants import cursor, link, JSON
...
class App(Ui_MainWindow):
    ...
    def fill_receipts(self):
        cursor.execute(f""""CALL GetReceiptsForDepartment(1); -- Замените 1 на фактический идентификатор вашего отделения связи{self.department_id}""")
        ...
        for receipt in self.receipts:
            self.listWidgetTabReceipts.addItem(f"Квитанция номер {receipt['id']} была выдана в {receipt['date']} на срок {receipt['time']} месяцев  на сумму {receipt['price']} рублей ")
    ...
    def fill_postmen(self):
        ...
        for postman in self.postmen:
            self.listWidget_TabPostmen.addItem(f"Почтальон {postman['first_name']} {postman['surname']} {postman['last_name']}")
    ...
    def update_lists(self):
        ...
        for subscriber in self.subscribers:
            self.listWidgetFioSubscriber.addItem(f"{subscriber['first_name']} {subscriber['last_name']} {subscriber['surname']}")
    ...
    def make_new_receipt(self):
        ...
        cursor.execute(f"INSERT INTO Подписчик(Фамилия, Имя, Отчество, Дом_id) VALUES('{last_name}', '{first_name}', '{surname}', {home_id});")
        link.commit()
        ...
        cursor.execute(f"INSERT INTO Квитанция (Срок_подписки, Дата_начала_подписки, Подписчик_id, Издание_id) VALUES({subscribe_time}, '{date}', {subscriber_id}, {release_id});")
        link.commit()
        self.update_lists()
    ...
    def find_max(self):
        ...
        self.labelResultForRequest.setText(f"Количесто изданий максимально на участке под названием '{cursor.fetchone()[1]}'")
    ...
    def find_average(self, id=None):
        ...
        for result in results:
            final_text += f"Средний срок подписки на издание под названием {result[1]} равен {result[-1]} byn.\n"
        ...
        for subscriber in self.subscribers:
            if (subscriber['first_name'] == self.listWidgetFioSubscriber.selectedItems()[0].text().split(" ")[0]
                ...
                for result in cursor.fetchall():
                    ...
                    count = result[4]
                    self.labelResultForRequest.setText(f"Индекс={index}, Название={name}, Цена={price}, Количество доступно={count}")
